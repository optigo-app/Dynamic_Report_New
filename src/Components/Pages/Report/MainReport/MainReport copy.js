...
export default function MainReport({
  ...
  const handleApplyFilter = () => {
    ...
    // Apply draft filters to screen
    setFilters(draftFilters);

    // Build activities for all draft filters
...
export default function MainReport({
  ...
  const unicSummaryColumns = columnsHide?.filter((col) => {
    ...
    return String(columnData?.UnicSummury).toLowerCase() === "true";
  });

  console.log("unicSummaryColumns", unicSummaryColumns);
    ...
    return (
      ...
      >
        {finalSummaryColumns.map((col) => {
          ...
          const isUniq =
            String(columnMeta?.UnicSummury).toLowerCase() === "true";
...
export default function MainReport({
  ...
  //   };
  //   const baseUrl = iframeModelData?.rd1[0]?.BaseUrl || "";
  //   const ReportRedirectUrl = iframeModelData?.rd1[0]?.ReportRedirectUrl || "";
  //   const queryParams = iframeModelData?.rd
  //     ?.map((item) => {
  ...
  //       } else {
  //         const dynamicValue =
  //           getRowValue(ParameterName) || ParameterValue || "";

  //         return `${ParameterName}=${dynamicValue}`;
  //       }
  //     })
  //     .join("&");
  //   return `${baseUrl}${ReportRedirectUrl}&${queryParams}`;
  // };
  ...
  const handleCellClick = (params) => {
    if (params?.colDef?.onHrefLinkModel === "true") {
      ...
      const baseUrl = navigationData?.rd1[0]?.BaseUrl || "";
      const redirectUrl = navigationData?.rd1[0]?.ReportRedirectUrl || "";
      const navigateName = params?.colDef?.onHrefNavigate || "";
      const queryParams = navigationData?.rd
        ?.map((item) => {
          const { VariableName, VariableValue, IsStatic } = item;
          if (IsStatic === "true") {
            return `${VariableName}=${encodeURIComponent(VariableValue)}`;
