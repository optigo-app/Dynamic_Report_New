...
const MakeNewReport = ({
    ...
    const hiddenReports = otherReport.slice(5);
    ...
    const mapColumnsForCurrentSave = (allColumnData) => {
        return allColumnData.map((col) => ({
            ColId: Number(col.ColId),
            IsHidden: col.IsVisible === "True" ? "True" : "False",
            ColumnOrder: col.DisplayOrder,
            ColumnAlias: col.HeaderName || "",
            ColumnWidth: col.Width,
        }));
    ...
    const handleSaveReport = async () => {
        ...
        const body = {
            ...
            p: JSON.stringify({
                ...
                Filters: [],
                Columns: mapColumnsForSave(allColumData),
            }),
        ...
        if (response?.rd?.[0]) {
            const updatedColumns = makeAllColumnsVisible(allColumData);
            const newSubReport = {
                SubReportId: statusObj.SubReportId,
                ReportId: reportId,
                SubReportName: subReportName.trim(),
                Filters: JSON.stringify([]),
                Columns: JSON.stringify(mapColumnsForCurrentSave(allColumData))
            };
            setOtherReprot(prev => [...prev, newSubReport]);
            setAllColumDataBack(updatedColumns);
            setErrorMessageColor('success');
            setOpenSnackbar(true);
            setSubReportName('');
